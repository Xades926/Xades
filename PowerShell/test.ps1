...
$testingPath = "C:\WorkDir\testing_custom"
$Global:testingOutPath = "C:\WorkDir\testing_custom"
...
function ExtractSolution(){
    ...
    Write-Output "`n[INFO][$Global:status/$Global:total] Starting excute ""$pyScriptPath"""
    ...
    Write-Output "[INFO][$Global:status/$Global:total] Finished excute ""$pyScriptPath"""
    ...
    Write-Output "`n[INFO][$Global:status/$Global:total] Starting decompile $Global:dllCnt dlls..."
    if (-not (Test-Path "$decompilePath\decompile.sln")) {    
        ...
        $process.WaitForExit()
        Write-Output "[INFO][$Global:status/$Global:total] Finished make solution file.. ""$decompilePath\decompile.sln""."
    }
    else{
        Write-Output "[WARNING][$Global:status/$Global:total] Already exist solution file.. ""$decompilePath\decompile.sln""."
    }
}
...
function AnalyzeSolution(){
    ...
    Write-Output "`n[INFO][$Global:status/$Global:total] Starting excution ""$exePath"".."
    if (-not (Test-Path $outputPath)) {
        New-Item -ItemType Directory -Path $outputPath | Out-Null
    }
   # Write-Output "-sln $slnPath -out $outputPath -conf $configurePath"
    ...
    # Write-Output "`n[INFO][$Global:status/$Global:total] Starting execute ""$pyScriptPath"".."
...
function CollectLog(){
    ...
    $process = Start-Process $pythonPath "$pyScriptPath $apkPath $Global:dllCnt result_.csv" -WindowStyle Hidden -PassThru -RedirectStandardOutput $pyLogPath  -RedirectStandardError $pyErrPath -Wait
    Write-Output "[INFO][$Global:status/$Global:total] Finished excution ""$pyScriptPath"""
...
function UnzipApkFile($apkPath) {
    ...
    Write-Output "`n[INFO][$Global:status/$Global:total] Starting unzip apk file ""$apkPath""."
    Start-Process bz "x -y $apkPath $unzipPath"  -WindowStyle Hidden -Wait
    Write-Output "[INFO][$Global:status/$Global:total] Finished unzip apk file."
}
...
function RunFlowDroid($apkPath){
    ...
    Write-Output "`n[INFO][$Global:status/$Global:total] Starting excution FlowDroid $apkPath."
    $process = Start-Process java "-cp $jarPath java_analyzer.Main --output $outputDir --ss C:\WorkDir\Xamarin-TaintAnalyzer\Configure\Java_SourcesAndSinks.txt --apk $apkPath --timeout 900" -WindowStyle Hidden -PassThru  -RedirectStandardOutput $logPath1  -RedirectStandardError $logPath2 -Wait
    Write-Output "[INFO][$Global:status/$Global:total] Finished excution FlowDroid."
    Stop-Process $process
}
#### MAIN ####

$apkPaths = Get-ChildItem -Path $testingPath -Filter *.apk
$Global:total = ($apkPaths).Count
...
foreach($apkPath in $apkPaths){
    ...
    Write-Output "`n[INFO][$Global:status/$Global:total] Starting Csharp Taint Analysis about ""$apkPath""."
    ExtractSolution;
    AnalyzeSolution;
